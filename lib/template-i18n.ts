...
import { KIND_ONLY_WORDS, KIND_ONLY_WORDS_EN } from "./template-kind-lexicon";
...
// kind 专属词只应出现在真的拥有该板块的站里。内容层会把同一子类用于多个版式，
// 因此不能靠手写 fragment 猜某个词最终是否安全；这里按 slug 的实际板块做确定性改写。
const KIND_WORD_REPLACEMENTS_ZH: Readonly<Record<string, string>> = {
  博客: "内容页",
  资讯: "信息",
  新闻: "近况",
  最新动态: "近期进展",
...
const KIND_WORD_REPLACEMENTS_EN: Readonly<Record<string, string>> = {
  blog: "updates",
  newsletter: "updates",
  ...
  menu: "food selection",
  "signature dish": "house favorite",
  "dine-in": "in-store",
  chef: "cook",
  "add to cart": "choose now",
  "in stock": "available now",
  "free shipping": "delivery included",
  checkout: "complete your request",
  subscription: "ongoing service",
  "billed monthly": "priced by period",
  "free trial": "introductory access",
  renewal: "continued service",
  "our team": "our specialists",
  founder: "lead",
  portfolio: "selected work",
  "case study": "project review",
  "photo gallery": "visual selection",
  milestones: "key moments",
  faq: "help",
  "frequently asked": "often raised",
};

function replaceEnglishWord(text: string, word: string, replacement: string): string {
  const escaped = word.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  return text.replace(new RegExp(escaped, "gi"), (match) => {
    if (!/^[A-Z]/.test(match)) return replacement;
    return replacement.charAt(0).toUpperCase() + replacement.slice(1);
  });
}

/** 把当前版式不存在的 kind 专属词换成行业中性说法。 */
export function copyForSectionKinds(
  text: string,
  availableKinds: ReadonlySet<SectionKind>,
  lang: Lang,
): string {
  const lexicon = lang === "en" ? KIND_ONLY_WORDS_EN : KIND_ONLY_WORDS;
  const replacements = lang === "en" ? KIND_WORD_REPLACEMENTS_EN : KIND_WORD_REPLACEMENTS_ZH;
  let result = text;
  if (lang === "zh" && !availableKinds.has("news")) {
    // 「订阅制」虽归 pricing，但同时含 news 的禁词「订阅」；整词先改才不会变成
    // 「定期配送制」这种机械拼接。
    result = result.split("订阅制").join(KIND_WORD_REPLACEMENTS_ZH["订阅制"]);
  }
  const rewrites: { word: string; replacement: string }[] = [];
  for (const [kind, words] of Object.entries(lexicon)) {
    if (availableKinds.has(kind as SectionKind)) continue;
    for (const word of words) {
      const replacement = replacements[word];
      if (!replacement) continue;
      rewrites.push({ word, replacement });
    }
  }
  // 全 kind 一起按长词优先，避免先把「订阅制」里的「订阅」换成生硬的半截词。
  for (const { word, replacement } of rewrites.sort((a, b) => b.word.length - a.word.length)) {
    result = lang === "en"
      ? replaceEnglishWord(result, word, replacement)
      : result.split(word).join(replacement);
  }
  return result;
}
...
export const UI: Record<string, Bi> = {
  ...
  contactLead: bi("请写下需求", "Tell us what you need"),
  trustedBy: bi("客户信赖", "Client trust"),
  ...
  genBy: bi("模板生成", "Made with OceanLeo"),
  proSolution: bi("专业方案", "Pro Solution"),
  // section 通用标题（会被行业化标题覆盖，作兜底）
  secFeatures: bi("为什么选择我们", "Why Choose Us"),
  secFeaturesSub: bi("六项选择理由", "Six clear strengths"),
  secServices: bi("我们的服务", "Our Services"),
  secProducts: bi("热门商品", "Featured Products"),
  secProductsSub: bi("严选放心购", "Curated with care"),
  secMenu: bi("招牌菜单", "Signature Menu"),
  secMenuSub: bi("好食材，好风味", "Fresh, made with care"),
  secGallery: bi("作品 / 案例展示", "Work & Showcase"),
  secGallerySub: bi("每项成果都用心", "Work made with care"),
  secCases: bi("成功案例", "Success Stories"),
  secCasesSub: bi("专业成果看得见", "Results you can inspect"),
  secTeam: bi("核心团队", "Our Team"),
  secTeamSub: bi("专业可靠有担当", "Skilled and accountable"),
  secPricing: bi("价格方案", "Pricing Plans"),
  secPricingSub: bi("灵活方案按需选", "Flexible plans"),
  secProcess: bi("我们怎么做", "How We Work"),
  secProcessSub: bi("步骤清楚可控", "Clear steps"),
  secFaq: bi("常见问题", "FAQ"),
  secFaqSub: bi("疑问都有解答", "Answers when needed"),
  secNews: bi("新闻资讯", "News & Insights"),
  secNewsSub: bi("近期动态", "Recent updates"),
  secTimeline: bi("发展历程", "Our Journey"),
  secTimelineSub: bi("一路积累向前", "Built step by step"),
  secMilestone: bi("发展里程碑", "Milestones"),
  secMilestoneSub: bi("关键节点见成长", "Moments that shaped us"),
  faqMore: bi("还有其他疑问？", "More questions?"),
  // 下方 chartFootnote 已明确说明是示例，这里不再重复一遍，避免与单位拼成共享长句。
  chartNote: bi("", ""),
  chartUnit: bi("单位", "Unit"),
  chartFootnote: bi("请替换示例值", "Replace sample values"),
  langToggle: bi("EN", "中"),
};
...
const SEC_BY_INDUSTRY: Record<string, Partial<Record<SecKey, SecTitle>>> = {
  business: {
    cases: { title: bi("典型案例", "Notable Cases"), sub: bi("专业实力可查", "Expertise on record") },
    team: { title: bi("专业团队", "Our Professionals") },
    process: { title: bi("服务流程", "Our Process") },
  },
  media: {
    cases: { title: bi("代表作品", "Featured Work"), sub: bi("作品见创意", "Ideas made visible") },
    team: { title: bi("创意团队", "Creative Team") },
    process: { title: bi("创作流程", "Creative Process") },
    gallery: { title: bi("作品精选", "Selected Work") },
  },
  fashion: {
    cases: { title: bi("上身实拍", "Lookbook"), sub: bi("每件都值得看见", "Made for the spotlight") },
  ...
  tech: {
    cases: { title: bi("落地案例", "Case Studies"), sub: bi("技术落地见实效", "Technology put to work") },
    team: { title: bi("技术团队", "Engineering Team") },
    process: { title: bi("交付流程", "Delivery Process") },
  },
  life: {
    cases: { title: bi("客片故事", "Client Stories"), sub: bi("用心记录相遇", "Moments kept with care") },
    gallery: { title: bi("影像展示", "Visual Showcase") },
    team: { title: bi("服务团队", "Our Team") },
  },
  food: {
    cases: { title: bi("食客好评", "Guest Reviews"), sub: bi("好味道看得见", "Flavor you can see") },
    products: { title: bi("招牌推荐", "House Picks") },
    gallery: { title: bi("门店实拍", "Inside the Restaurant") },
    team: { title: bi("餐饮团队", "Restaurant Staff") },
  },
  industry: {
    cases: { title: bi("供货案例", "Supply Cases"), sub: bi("交付能力可查", "Delivery on record") },
  ...
  logistics: {
    cases: { title: bi("服务案例", "Service Cases"), sub: bi("时效覆盖有保障", "Reach you can rely on") },

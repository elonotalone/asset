...
console.log("\n③ 图表片段能不能摊平成可画的数列（oceanleo.chart.v1 的 dataset + option）");

const chartFragment = path.join(process.cwd(), "content", "works", "chart.json");
if (!existsSync(chartFragment)) {
  console.log("  （还没有 content/works/chart.json）");
} else {
  const { chartSpecOf } = await import("../components/WorksViewer.tsx");
  for (const w of JSON.parse(readFileSync(chartFragment, "utf8"))) {
    const src = path.join(process.cwd(), "public", w.view?.src ?? "");
    if (!existsSync(src)) continue;
    const spec = chartSpecOf(JSON.parse(readFileSync(src, "utf8")));
    if (!spec) {
      failures++;
      console.log(`  FAIL  ${w.id} —— 画不出来，只能退回封面`);
      continue;
    }
    console.log(`  ok    ${w.id}  ${spec.series.map((s) => `${s.name}(${s.type}×${s.points.length})`).join(" ")}`);
  }
}

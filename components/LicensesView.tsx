...
export function LicensesView() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-5 py-8">
      <h1 className="text-2xl font-semibold text-zinc-900">{tt("授权说明")}</h1>
      <p className="mt-2 text-sm text-zinc-500">
        {tt("素材库聚合多个开源素材源，每个素材都标注了具体授权。库内默认只展示")}
        <span className="font-medium text-zinc-700">{tt("「可商用」")}</span>
        {tt("素材；切换过滤可进一步只看「可改编」或查看全部（含仅非商用）。")}
      </p>

      <h2 className="mt-8 text-lg font-semibold text-zinc-800">{tt("素材来源")}</h2>
        ...
        <table className="w-full text-left text-sm">
          <thead className="bg-zinc-50 text-xs uppercase text-zinc-500">
            <tr>
              ...
              <th className="px-4 py-2">{tt("主要授权")}</th>
              <th className="px-4 py-2">{tt("说明")}</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-zinc-100">
            {SOURCES.map((s) => (
              <tr key={s.name}>
                ...
                <td className="px-4 py-2 text-zinc-600">{tt(s.license)}</td>
                <td className="px-4 py-2 text-zinc-500">{tt(s.note)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <h2 className="mt-8 text-lg font-semibold text-zinc-800">{tt("授权类型对照")}</h2>
      <div className="mt-3 overflow-hidden rounded-xl border border-zinc-200">
        <table className="w-full text-left text-sm">
          <thead className="bg-zinc-50 text-xs uppercase text-zinc-500">
            <tr>
              <th className="px-4 py-2">{tt("授权")}</th>
              <th className="px-4 py-2 text-center">{tt("可商用")}</th>
              <th className="px-4 py-2 text-center">{tt("可改编")}</th>
              <th className="px-4 py-2 text-center">{tt("需署名")}</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-zinc-100">
            {CODES.map((c) => (
              <tr key={c.code}>
                <td className="px-4 py-2 font-medium text-zinc-800">{tt(c.code)}</td>
      ...
      <p className="mt-6 text-xs text-zinc-400">
        {tt("需署名的素材（CC-BY 系列）在详情页提供一键复制的署名文本，请在最终作品中保留。本库仅做聚合与授权归一化展示，最终合规责任随各素材原始授权条款。")}
      </p>
    </div>
  );
}
